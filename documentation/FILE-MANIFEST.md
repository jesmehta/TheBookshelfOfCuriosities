...
| `projects/christie/` | "Agatha Christie — Murder, She Wrote" — timeline + real-coastline map atlas + Short Stories tab, all in one page (`christie-timeline-v3.html`); `christie-atlas-v2.html` is a superseded standalone predecessor, kept for the record but not live. Also has its own `documentation.md` (design/font/changelog record) and `conversation-christie.md` (the reasoning behind it), the pre-repo conversation's own `christie-project-history.md`/`christie-timeline-changelog.md`, the two bibliography research tables this project started from (`agatha-christie-location-timeline.md`/`-v2.md`), and `fonts/`. |

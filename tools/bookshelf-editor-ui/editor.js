/* Bookshelf editor -- browser UI. Talks to tools/bookshelf-editor.js's
   /api/* routes; never touches the TSV files itself. Every mutating
   action refetches /api/state afterward rather than patching local
   state, so the UI can never drift from what's actually on disk (or from
   validation problems a *different* row's edit just introduced -- e.g.
   renaming a section id invalidates every entry that referenced the old
   id). Modeled on CabinetOfCuriosities/tools/cabinet-editor-ui/editor.js
   and form-follows-fx/tools/fffx-editor-ui/editor.js: no reserved/
   collapsible-panel columns, plus a Build tab. */
...
// A row's textareas each had their own independent CSS resize handle, so
// dragging one taller left its row siblings at their old, now-mismatched
// height. Syncing every textarea in a row to whichever one is tallest --
// on any of their resizes -- keeps the row looking like one row instead
// of a ransom note.
...
const ENTRY_SELECT_MAP = { status: STATUS_OPTIONS, location: LOCATION_OPTIONS, span: SPAN_OPTIONS };
...
document.getElementById("add-entry").addEventListener("click", () => {
  ...
  runMutation(() => apiCall("POST", "/api/entries", { status: "wip", section: firstSection }));

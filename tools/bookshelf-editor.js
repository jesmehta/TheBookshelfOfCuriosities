...
// CabinetOfCuriosities/tools/cabinet-editor.js (TSV CRUD),
// tools/admin-controls.js (the runScript/apiRun build-button pattern),
// and form-follows-fx/tools/fffx-editor.js (the one-page version of both).
...
function readAllSections() {
  return readSections(fs.readFileSync(SECTIONS_TSV_PATH, "utf8"), "content/bookshelf-sections.tsv");
}
function writeAllSections(rows) {
  fs.writeFileSync(SECTIONS_TSV_PATH, writeSections(rows), "utf8");
}
function readAllEntries() {
  return readEntries(fs.readFileSync(ENTRIES_TSV_PATH, "utf8"), "content/bookshelf-entries.tsv");
}
...
// "rebuild content" and an mkdocs strict sanity check.
...
function apiMkdocsCheck(res) {
  const tmpSiteDir = path.join(os.tmpdir(), `bookshelf-mkdocs-check-${Date.now()}`);
...
server.listen(PORT, "127.0.0.1", () => {
  console.log(`Bookshelf editor running at http://127.0.0.1:${PORT}/admin/`);

# Bookshelf of Curiosities â€” Design System

**This file defines the target visual system.** When anything else in the repo conflicts with it â€” existing CSS, old class names, MkDocs Material defaults â€” this file wins. Do not blend the old system with this one. Do not produce hybrids.

---

## Context: what this replaces

The existing `docs/stylesheets/bookshelf-landing.css` is the **old system** (V3, Fraunces-based, gallery-wall card metaphor, frame moods). It is being replaced in full. The new system uses a different font stack, a different card pattern, and different layout components. When you see `.bookshelf-frame`, `.bookshelf-image-wrap`, `frameMood`, `randomFrameRatio()`, `frameWidthFraction()` â€” those are the old system. Do not carry them forward.

---

## Site architecture (unchanged)

MkDocs Material site. Landing page is `docs/index.md`. All landing CSS goes in `docs/stylesheets/bookshelf-landing.css`. JS files live in `docs/assets/js/`. Standalone projects (`scifi/`, `asimov/`, etc.) are plain HTML/CSS/JS at the repo root, copied into `public/` by CI â€” MkDocs never touches them.

Raw colour/font values live in one place, `docs/stylesheets/bookshelf-tokens.css` (`:root`-scoped `--bookshelf-*` custom properties, plus the Google Fonts `@import`) â€” both `bookshelf-landing.css` (the landing page) and `docs/stylesheets/bookshelf-material.css` (Material's `--md-*` variables, for every page including the chrome around the landing page itself) read from it instead of hardcoding the same values twice. Change a value once in tokens.css; both flows pick it up. See "Fonts" and "Colour tokens" below for what each token does, and `README.md`'s changelog for why this split happened (a font/palette mismatch had been live between the landing page and the rest of the site).

The landing page must continue to work inside MkDocs Material's shell. All CSS rules must be scoped under `.bookshelf-landing` to avoid clashing with Material's own styles. The MkDocs header must be hidden on the landing page (existing mechanism: `body:has(.bookshelf-landing) .md-header` and `body.bookshelf-landing-page .md-header { display: none !important }` â€” keep this).

---

## Fonts

Load via Google Fonts. Exactly these four, no substitutions. The `@import` lives in `docs/stylesheets/bookshelf-tokens.css`, not `bookshelf-landing.css` â€” loaded once, site-wide, since the font stacks are now shared with the rest of the site (see below).
...
|---|---|---|
| `--ff-serif` | Libre Baskerville | Primary display â€” hero "Bookshelf", card titles, ghost letters |
| `--ff-inst` | Instrument Serif (italic) | Flowing accent â€” "The", "Curiosities", card body text, descriptions |
| `--ff-sans` | Syne | Labels only â€” the word "of" in the hero title, section names |
| `--ff-mono` | Syne Mono | All mono detail â€” kicker, section numbers, tags, ticker, card category |

Of these four, only two extend beyond the landing page: `mkdocs.yml`'s `theme.font` sets `text: Libre Baskerville` / `code: Syne Mono` for every Material-rendered page, since Material's font config only has two slots and `--ff-inst`/`--ff-sans` are landing-page-only decorative faces. Kept in sync with `bookshelf-tokens.css` *by hand* â€” plain YAML can't reference a CSS custom property.

---

## Colour tokens

Raw values live in `docs/stylesheets/bookshelf-tokens.css` (`--bookshelf-*`, `:root`-scoped â€” loaded on every page, not just the landing page). `bookshelf-landing.css` maps those onto the `.bookshelf-landing`-scoped names below (`--ink: var(--bookshelf-ink)`, etc.); `docs/stylesheets/bookshelf-material.css` maps the *same* tokens onto Material's `--md-*` variables for the rest of the site. Never hardcode a hex value directly in either consuming file â€” extend `bookshelf-tokens.css` first. Never introduce new colour values.
...
- No fill, no SVG, no image â€” just the letterform outline.
- Hero ghost: `position: absolute; top: -4%; right: -6%`, `font-size: clamp(280px, 38vw, 520px)`, animated with `ghostDrift` (18s, alternate, ease-in-out: translateY 0â†’20px, rotate -1degâ†’0.5deg).
...
All four spans animate in with `riseIn` (opacity 0â†’1, translateY 18pxâ†’0) with staggered delays: 0.35s, 0.5s, 0.7s, 0.75s.
...
### Dataviz block
A wide feature block with a large ghost word "DATA" rotated 90Â° as a background texture. Contains a kicker (Syne Mono, `--gold`), a display title (Libre Baskerville bold italic, `clamp(28px, 5vw, 58px)`), a description (Instrument Serif italic), and chip labels.
...
### Card grid
12-column CSS grid, 9px gap. Span classes: `.c4` (span 4), `.c5` (span 5), `.c6` (span 6), `.c7` (span 7), `.c8` (span 8), `.c12` (span 12). On mobile â‰¤720px all cards collapse to `span 12`.
...
<a href="URL" class="card c7">               <!-- live card -->
  ...
  <div class="card-inner">
    <span class="badge-live">Live â†—</span>   <!-- only on live cards -->
    <p class="card-cat">Category Â· Type</p>
    <h2 class="card-title">Title</h2>
    <p class="card-body-text">Description.</p>
    <div class="card-foot">
      <span class="card-tag">Tag Â· Label</span>
      <span class="card-arrow">â†—</span>
    </div>
  </div>
</a>

<div class="card c5 card-dormant">           <!-- dormant card â€” div not a -->
  ...
  <div class="card-inner">
    ...
    <div class="card-foot">
      <span class="card-tag">Tag Â· Label</span>
      <span class="card-arrow" style="opacity:.25">â€”</span>
    </div>
  </div>
</a>
```
...
- Gold top-bar: `::after` pseudo, `height: 2px`, `background: var(--amber)`, `scaleX(0â†’1)` on hover from left
...
- No hover effects, no pointer events â€” `pointer-events: none`
...
- Arrow replaced with `â€”` at `opacity: 0.25`
...
- **Dot**: 5px amber circle, `mix-blend-mode: screen`, zero lag â€” always exactly at pointer position
...
- Bail out on `pointer: coarse` and `prefers-reduced-motion` â€” cursor stays native, `cursor: none` is never set unconditionally
- `body.bookshelf-landing-page .md-header { display: none !important }` is set by the cursor JS (adds class to body) â€” preserve this
...
Fixed-position canvas behind all content (`z-index: 0`, `pointer-events: none`), attached to `#p5wrap`. As of V4.1 this is a firefly state-machine sim (~24 motes, noise-driven heading, flying/landing/resting/takeoff states) rather than the drifting-mote physics described in earlier versions of this doc â€” see `README.md`'s changelog (V4.1) and `LANDING-PAGE-NOTES.md` bug #5 for why. On `windowResized`, canvas resizes to match viewport.
...
`bookshelf-gallery.js` groups entries by `section`, renders a `.sec-head` for each visible section, then renders matching entries as cards. Section numbering (i, ii, iiiâ€¦) is derived from `bookshelfSections[].order`.
...
- Do not use `.bookshelf-frame`, `.bookshelf-image-wrap`, `.bookshelf-card-inner`, `frameMood`, `frame-*` classes â€” those are the old system
- Do not use `randomFrameRatio()` or `frameWidthFraction()` â€” old system
...
- Do not create or commit a `public/` folder â€” CI only
